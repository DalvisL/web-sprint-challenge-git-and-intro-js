...
function addArtist(arr, artName, artYears, artGenre, artNation, artBio) {
  arr.push({name: artName, years: artYears, genre: artGenre, nationality: artNation, bio: artBio})
  return arr;
}
...
function artistByCountry(arr, artNation){
  const newArr = [];
  for(let i = 0; i < arr.length; i++) {
    if(arr[i].nationality === artNation && !(arr[i].nationality.includes(','))) {
      newArr.push(arr[i].name)
    }
  }
  return newArr;
